...
import { getAdvertiserReport, ReportData, ReportParams } from '@/api/report'
import {
...
export default function ReportAdvertiser() {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const data = await getAdvertiserReport(params)
      setReportData(data)
    } catch (error) {
      console.error('Failed to fetch advertiser report:', error)
      showError('获取广告主报表数据失败')
      setReportData([])
    } finally {
      setLoading(false)
    }
