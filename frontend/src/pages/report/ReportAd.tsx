...
import { getAdReport, ReportData, ReportParams } from '@/api/report'
import {
...
export default function ReportAd() {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const data = await getAdReport(params)
      setReportData(data)
    } catch (error) {
      console.error('Failed to fetch ad report:', error)
      showError('获取广告计划报表数据失败')
      setReportData([])
    } finally {
      setLoading(false)
    }
