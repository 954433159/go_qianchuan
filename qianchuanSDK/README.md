...
oauthUrl := manager.OauthConnect(qianchuanSDK.OauthParam{
    ...
    Scope:       []int64{20120000, 22000000}, // 权限范围
    MaterialAuth: "1",
...
```go
accessToken, err := manager.OauthAccessToken(qianchuanSDK.OauthAccessTokenReq{
    AuthCode: "AUTH_CODE",
})
```
...
```go
manager.OauthRefreshToken(qianchuanSDK.OauthRefreshTokenReq{
...
```go
// 注意：千川API使用OauthRefreshToken来刷新token
manager.OauthRefreshToken(qianchuanSDK.OauthRefreshTokenReq{
    RefreshToken: "REFRESH_TOKEN",
})
```
...
```go
// 注意：千川使用UserInfo接口
userInfo, err := manager.UserInfo(qianchuanSDK.UserInfoReq{
