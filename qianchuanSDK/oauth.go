...
func (m *Manager) OauthConnect(param OauthParam) (string, error) {
	scope, err := json.Marshal(param.Scope)
	if err != nil {
		return "", fmt.Errorf("序列化Scope失败: %w", err)
	}
	return m.url("%s?app_id=%d&state=%s&scope=%s&material_auth=%s&redirect_uri=%s&rid=%s", conf.API_OAUTH_CONNECT,
		param.AppId, param.State, string(scope), param.MaterialAuth, param.RedirectUri, param.Rid), nil
}
